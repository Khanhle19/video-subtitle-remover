...
class STTNInpaint:
    def __init__(self):
        ...
        # 1. Create InpaintGenerator model instance and load it to the selected device
        ...
        self.model.eval()
        # Width and height for model input
        self.model_input_width, self.model_input_height = 640, 120
        # 2. Set neighbor frame number
    ...
    def __call__(self, input_frames: List[np.ndarray], input_mask: np.ndarray):
        ...
        :param input_mask: Subtitle area mask
        """
        ...
        H_ori = int(H_ori + 0.5)
        W_ori = int(W_ori + 0.5)
        # Determine vertical height part for subtitle removal
        ...
        for j in range(len(frames_hr)):
            image = frames_hr[j]
            # Cut and scale for each removal part
            for k in range(len(inpaint_area)):
                image_crop = image[inpaint_area[k][0]:inpaint_area[k][1], :, :]  # Cut
                image_resize = cv2.resize(image_crop, (self.model_input_width, self.model_input_height))  # Scale
                frames_scaled[k].append(image_resize)  # Add scaled frame to corresponding list
    ...
    def get_ref_index(self, neighbor_ids, length):
        ...
        ref_index = []
        # Iterate gradually based on ref_length within video length range
    ...
    def inpaint(self, frames: List[np.ndarray]):
        ...
        # Move feature tensors to specified device (CPU or GPU)
        feats = feats.to(self.device)
        # Initialize a list with same length as video to store completed frames
        comp_frames = [None] * frame_length
        # Disable gradient calculation to save memory and accelerate during inference
        with torch.no_grad():
            # Pass processed frames through encoder to generate feature representations
        ...
        # Get repainting area
        # Loop process video within set neighbor frame stride
        for f in range(0, frame_length, self.neighbor_stride):
            ...
            with torch.no_grad():
                ...
                # Generate image from predicted features through decoder, apply tanh activation, and detach tensor
                pred_img = torch.tanh(self.model.decoder(pred_feat[:len(neighbor_ids), :, :, :])).detach()
                # Rescale result tensor to 0-255 range (image pixel values)
                pred_img = (pred_img + 1) / 2
                # Move tensor back to CPU and convert to NumPy array
                pred_img = pred_img.cpu().permute(0, 2, 3, 1).numpy() * 255
                # Traverse neighbor frames
                for i in range(len(neighbor_ids)):
                    idx = neighbor_ids[i]
                    # Convert predicted image to unsigned 8-bit integer format
                    img = np.array(pred_img[i]).astype(np.uint8)
                    if comp_frames[idx] is None:
                        # If this position is empty, assign newly calculated image
                        comp_frames[idx] = img
                    else:
                        # If there is already an image at this position, blend new and old images to improve quality
    ...
    def get_inpaint_area_by_mask(H, h, mask):
        """
        Get subtitle removal area, determine area and height to fill based on mask
        """
        # List to store painting areas
        inpaint_area = []
        # Start from subtitle position at bottom of video, assuming subtitles are usually at the bottom
        to_H = from_H = H
        ...
        while from_H != 0:
            if to_H - h < 0:
                ...
                # Determine segment upper boundary
                from_H = to_H - h
            ...
            to_H -= h
        return inpaint_area  # Return painting area list
    ...
    def get_inpaint_area_by_selection(input_sub_area, mask):
        ...
        # Check if last interval reached maximum
        if inpaint_area[-1][1] != ymax:
            # If not, create a new interval, starting from end of last interval, ending at expanded value
...
class STTNVideoInpaint:
    ...
    def read_frame_info_from_video(self):
        ...
        # Get video width, height, fps and frame count info and store in frame_info dictionary
        frame_info = {
            'W_ori': int(reader.get(cv2.CAP_PROP_FRAME_WIDTH) + 0.5),  # Original width of video
            'H_ori': int(reader.get(cv2.CAP_PROP_FRAME_HEIGHT) + 0.5),  # Original height of video
            'fps': reader.get(cv2.CAP_PROP_FPS),  # Frame rate of video
            'len': int(reader.get(cv2.CAP_PROP_FRAME_COUNT) + 0.5)  # Total frames of video
        }
    ...
    def __init__(self, video_path, mask_path=None, clip_gap=None):
        ...
        # Configure max frames loadable in one processing
        if clip_gap is None:
    ...
    def __call__(self, input_mask=None, input_sub_remover=None, tbar=None):
        ...
        try:
            ...
            for i in range(rec_time):
                ...
                frames = {}  # Frame dictionary to store cropped images
                comps = {}  # Combination dictionary to store repaired images
                ...
                # If no valid frames read, skip current iteration
                    ...
                    for j in range(valid_frames_count):
                        ...
                        for k in range(len(inpaint_area)):
                            if j < len(comps[k]):  # Ensure index is valid
                                # Re-expand repaired image to original resolution and blend into original frame
